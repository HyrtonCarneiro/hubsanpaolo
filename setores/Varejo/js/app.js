...
function iniciarListenerEquipe() {
    try {
        const qEquipe = query(collection(db, "varejo_equipe"), orderBy("nome"));
...
window.adicionarMembro = async function() {
    ...
    try {
        await addDoc(collection(db, "varejo_equipe"), { nome });
...
window.removerMembro = async function(idMembro, nomeMembro) {
    ...
    try {
        await deleteDoc(doc(db, "varejo_equipe", idMembro));
