...
function initApp() {
    ...
    if (typeof window.initLinksListeners === 'function') window.initLinksListeners('Financeiro');
...
function iniciarListenerEquipe() {
    try {
        const qEquipe = query(collection(db, "financeiro_equipe"), orderBy("nome"));
...
window.adicionarMembro = async function() {
    ...
    try {
        await addDoc(collection(db, "financeiro_equipe"), { nome });
...
window.removerMembro = async function(idMembro, nomeMembro) {
    ...
    try {
        await deleteDoc(doc(db, "financeiro_equipe", idMembro));
