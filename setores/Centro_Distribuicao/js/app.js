// js/app.js - Centro de Distribuição
// Entry point for Centro de Distribuição sector
...
function initApp() {
    ...
    if (window.CoreUI) {
        ...
        if (window.renderDynamicSidebar) {
            window.renderDynamicSidebar('sidebar-container', {
                sectorTitle: 'Centro de Distribuição',
    ...
    if (window.EquipeController) {
        window.EquipeController.init('centro_distribuicao_equipe');
    }

    // Initialize Links Listener
    if (typeof window.initLinksListeners === 'function') {
        window.initLinksListeners('Centro_Distribuicao');
    }
